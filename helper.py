...
def draw_rect(start_x: int, start_y: int, width=cell_size, height=cell_size):
    el = Element("rect")
    el.add_attribute("x", str(start_x))
    el.add_attribute("y", str(start_y))
    el.add_attribute("width", str(width))
    el.add_attribute("height", str(height))
    return el


def draw_circle(start_x, start_y, width=cell_size):
    el = Element('circle')
    el.add_attribute('cx', str(start_x+width/2))
    el.add_attribute('cy', str(start_y+width/2))
...
def draw_rect_code(matrix: list[list[bool]]) -> Element:
    ...
    for index_y, row in enumerate(matrix):
        for index_x, el in enumerate(row):
            if el:
                rect = draw_rect(index_x*24, index_y*24, 24, 24)
                g.append_child(rect)
    return g
